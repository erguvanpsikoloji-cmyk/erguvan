        ...
        try {
            ...
            if (empty($image)) {
                if (isset($_FILES['image']) && $_FILES['image']['error'] !== UPLOAD_ERR_OK && $_FILES['image']['error'] !== UPLOAD_ERR_NO_FILE) {
                    $phpFileUploadErrors = [
                        1 => 'Dosya çok büyük (ini).', 2 => 'Dosya çok büyük (form).', 3 => 'Kısmi yükleme.', 
                        4 => 'Dosya yok.', 6 => 'Geçici klasör eksik.', 7 => 'Yazma hatası.'
                    ];
                    $err = $phpFileUploadErrors($_FILES['image']['error']] ?? 'Bilinmeyen';
                    if (empty($_POST['image_base64'])) throw new Exception("Görsel yüklenemedi: $err");
                } elseif (empty($_POST['image_base64'])) {
            ...
            // Boş alan kontrolü
            $missing_fields = [];
            if (empty($title)) $missing_fields[] = 'Başlık';
            if (empty($slug)) $missing_fields[] = 'Slug';
            if (empty($excerpt)) $missing_fields[] = 'Özet';
            if (empty($content) || $content === '<p><br></p>') $missing_fields[] = 'İçerik';
            if (empty($image)) $missing_fields[] = 'Görsel';
            if (empty($category)) $missing_fields[] = 'Kategori';
            ...
            $result = $stmt->execute([
                ':title' => $title, ':slug' => $slug, ':excerpt' => $excerpt, ':meta_description' => $meta_description,
                ':meta_title' => $_POST['meta_title'] ?? null, ':tags' => $tags, ':toc_data' => $toc_data, ':faq_data' => $faq_data,
                ':content' => $content, ':image' => $image, ':image_alt' => $image_alt, ':category' => $category,
                ':reading_time' => $reading_time, ':keywords' => $keywords, ':instagram_share' => $instagram_share,
                ':canonical_url' => $canonical_url, ':og_title' => $_POST['og_title'] ?? null, 
                ':og_description' => $_POST['og_description'] ?? null, ':schema_type' => $_POST['schema_type'] ?? 'BlogPosting'
            ]);

            if ($result) {
                file_put_contents(__DIR__ . '/blog_debug_trace.txt', "✅ DB Insert Success!\n", FILE_APPEND);
                $success = true;
                // Redirect KALDIRILDI - Mesaj göster
                // $_POST temizlenebilir veya yeni yazı için link verilebilir
            } else {
...
<?php if ($success): ?>
    <div style="background: #10b981; color: #fff; padding: 20px; border-radius: 12px; margin-bottom:30px; border: 2px solid #059669;">
        <h3 style="margin:0 0 10px 0;">✨ Harika! Yazı başarıyla yayınlandı.</h3>
        <p style="margin:0;">Yayındaki blog yazısını görmek veya listeye dönmek için aşağıdaki butonları kullanabilirsiniz.</p>
        <div style="margin-top:15px;">
            <a href="blog.php" class="btn btn-light" style="color:#059669; font-weight:bold;">📋 Listeye Dön</a>
            <a href="<?php echo url('blog/'.$slug); ?>" target="_blank" class="btn btn-light" style="color:#059669; font-weight:bold;">👁️ Yazıyı Görüntüle</a>
            <a href="blog-add.php" class="btn btn-light" style="color:#059669;">➕ Yeni Yazı Ekle</a>
        </div>
    </div>
<?php endif; ?>

<?php if ($error): ?>
    <div style="background: #fee2e2; color: #b91c1c; padding: 15px; border-radius: 12px; margin-bottom:30px; border: 1px solid #fca5a5;">
        ⚠️ <?php echo $error; ?>
    </div>
<?php endif; ?>

<!-- FORM BAŞLANGICI (Başarılı olsa bile formu tekrar gösteriyoruz ama boşaltmak gerekebilir veya user düzeltme yapmak isteyebilir) -->
<form method="POST" id="blogForm" enctype="multipart/form-data">
    ...
    <div class="editor-container">
        ...
        <div class="col-left">
            <div class="admin-card">
                ...
                <div class="form-group">
                    ...
                    <div class="input-group">
                        <input type="text" name="slug" id="slug" class="form-control" required value="<?php echo htmlspecialchars($_POST['slug'] ?? ''); ?>">
                        <button type="button" class="btn btn-secondary" onclick="generateSlug()"><i class="fas fa-sync"></i> Otomatik Oluştur</button>
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label">Kısa Özet (Meta Description)</label>
                    <textarea name="excerpt" id="excerpt" class="form-control" rows="3" required maxlength="300"><?php echo htmlspecialchars($_POST['excerpt'] ?? ''); ?></textarea>
                </div>
                <div class="form-group">
                    <label class="form-label">İçerik</label>
                    <div id="quill-toolbar">
                         <!-- Toolbar Items -->
                         <span class="ql-formats"><select class="ql-header"><option value="2"></option><option value="3"></option><option selected></option></select></span>
                         <span class="ql-formats"><button class="ql-bold"></button><button class="ql-italic"></button><button class="ql-link"></button></span>
                    ...
                    <div id="phpContentData" style="display:none;"><?php echo htmlspecialchars($_POST['content'] ?? ''); ?></div>
                </div>
            </div>
            ...
            <div class="admin-card">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    <div>
                        <label>İçindekiler <button type="button" class="btn-magic" onclick="scanTOC()">🪄 Tara</button></label>
                        <div id="tocList" style="margin-top: 10px;"></div>
                        <input type="hidden" name="toc_data" id="tocHidden" value="<?php echo htmlspecialchars($_POST['toc_data'] ?? ''); ?>">
                    </div>
                    <div>
                        <label>SSS / FAQ <button type="button" class="btn-magic" onclick="scanFAQ()">🪄 Tara</button></label>
                        <div id="faqList" style="margin-top: 10px;"></div>
                        <input type="hidden" name="faq_data" id="faqHidden" value="<?php echo htmlspecialchars($_POST['faq_data'] ?? ''); ?>">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-right">
            <div class="admin-card">
                ...
                <div class="seo-list">
                    <div class="seo-item"><div id="light-title" class="dot"></div> <span>Başlık Uzunluğu</span></div>
                    <div class="seo-item"><div id="light-meta" class="dot"></div> <span>Meta Açıklama</span></div>
                    <div class="seo-item"><div id="light-content" class="dot"></div> <span>Anahtar Kelime</span></div>
                </div>
                <div class="form-group" style="margin-top: 15px;">
                    <label>Odak Anahtar Kelime</label>
                    <input type="text" id="focus_keyword" name="keywords" class="form-control" placeholder="Örn: sınav kaygısı" value="<?php echo htmlspecialchars($_POST['keywords'] ?? ''); ?>">
                </div>
            </div>
            ...
            <div class="admin-card">
                ...
                <div class="form-group">
                    ...
                    <input type="hidden" name="image_base64" id="image_base64" value="<?php echo htmlspecialchars($_POST['image_base64'] ?? ''); ?>">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="image_alt" class="form-control" placeholder="Resimde ne görünüyor?" value="<?php echo htmlspecialchars($_POST['image_alt'] ?? ''); ?>">
                </div>
            </div>
            ...
            <div class="admin-card">
                ...
                <div class="form-group">
                    ...
                    <select name="category" class="form-control" required>
                        ...
                        <?php foreach ($common_categories as $ca): ?>
                            <option value="<?php echo $ca; ?>" <?php echo ($_POST['category']??'') == $ca ? 'selected' : ''; ?>><?php echo $ca; ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                    <label>Etiketler</label>
                    <input type="text" name="tags" class="form-control" placeholder="Virgülle ayırın" value="<?php echo htmlspecialchars($_POST['tags'] ?? ''); ?>">
                </div>
                <div class="form-group">
                ...
                <div class="form-group" style="margin-top:10px;">
                    ...
                    <input type="url" name="canonical_url" class="form-control" placeholder="https://..." value="<?php echo htmlspecialchars($_POST['canonical_url'] ?? ''); ?>">
                </div>
            </div>
            ...
            <div class="admin-card">
                <div class="card-title">👤 Yazar Notu</div>
                 <div style="display:grid; gap:10px;">
                     <button type="button" class="btn btn-secondary w-100" onclick="fillSena()">👩‍⚕️ Uzm. Psk. Sena Ceren</button>
                     <button type="button" class="btn btn-secondary w-100" style="background:#3b82f6; color:white;" onclick="fillSedat()">👨‍⚕️ Uzm. Psk. Sedat Parmaksız</button>
                 </div>
            </div>
        </div>
    </div>
    ...
    <div class="form-footer">
        <button type="submit" class="btn btn-primary" style="background:#ec4899; border:0; padding:12px 25px;">🚀 Yazıyı Yayınla</button>
...
<script>
    ...
    // Basitleştirilmiş (Placeholder) Fonksiyonlar
    function scanTOC() { alert('Başlıklar tarandı!'); }
    function scanFAQ() { alert('Sorular tarandı!'); }
    function fillSena() { quill.clipboard.dangerouslyPasteHTML(quill.getLength(), '<div>Sena Ceren Notu...</div>'); }
    function fillSedat() { quill.clipboard.dangerouslyPasteHTML(quill.getLength(), '<div>Sedat Parmaksız Notu...</div>'); }
    ...
    function livePreview() {
         const d = { title:document.getElementById('title').value, content: quill.root.innerHTML };
