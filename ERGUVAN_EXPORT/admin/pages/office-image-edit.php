...
// Ofis görselini getir
$stmt = $db->prepare("SELECT * FROM office_images WHERE id = ?");
$stmt->execute([$id]);
$office_image = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $image = $_POST['image'] ?? $office_image['image'];
    ...
    if ($image && !$error) {
        try {
            $stmt = $db->prepare("UPDATE office_images SET 
                                   title = :title, 
...
$page = 'office-images';
$page_title = 'Ofis Görseli Düzenle';
require_once __DIR__ . '/../includes/header.php';
?>

<div class="edit-header">
    <div>
        <h2 style="margin: 0 0 5px 0;">Ofis Görseli Düzenle</h2>
        <p style="color: #64748b; margin: 0;">ID: <?php echo $office_image['id']; ?> | Oluşturulma: <?php echo date('d.m.Y H:i', strtotime($office_image['created_at'])); ?></p>
    </div>
</div>
...
<form method="POST" action="" id="officeImageForm" enctype="multipart/form-data">
    <?php echo csrfField(); ?>
    
    <div class="form-group">
        <label for="title">Başlık (Opsiyonel)</label>
        <input type="text" id="title" name="title" class="form-control" value="<?php echo htmlspecialchars($office_image['title'] ?? ''); ?>" placeholder="Örn: Bekleme Odası">
    </div>
    
    <div class="form-group">
        <label for="description">Açıklama (Opsiyonel)</label>
        <textarea id="description" name="description" class="form-control" rows="3" placeholder="Görsel hakkında kısa bir açıklama"><?php echo htmlspecialchars($office_image['description'] ?? ''); ?></textarea>
    </div>
    ...
    <div class="form-group">
        ...
        <div class="image-upload-wrapper">
            ...
            <input type="hidden" id="image" name="image" value="<?php echo htmlspecialchars($office_image['image']); ?>" required>
            <div id="uploadStatus" style="margin-top: 10px; display: none;"></div>
        </div>
        <div id="imagePreview" style="margin-top: 10px; <?php echo $office_image['image'] ? 'display: block;' : 'display: none;'; ?>">
            <img id="previewImg" src="<?php echo htmlspecialchars($office_image['image']); ?>" alt="Önizleme" style="max-width: 300px; max-height: 200px; border-radius: 8px; border: 2px solid #e2e8f0;">
        </div>
    ...
    <div class="form-row">
        <div class="form-group">
            ...
            <input type="number" id="display_order" name="display_order" class="form-control" value="<?php echo $office_image['display_order']; ?>" min="0">
        ...
        <div class="form-group">
            <label style="display: flex; align-items: center; gap: 10px; margin-top: 30px;">
                <input type="checkbox" name="is_active" <?php echo $office_image['is_active'] ? 'checked' : ''; ?>>
                <span>Aktif</span>
            </label>
        </div>
    </div>
