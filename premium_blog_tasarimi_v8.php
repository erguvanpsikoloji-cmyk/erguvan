<?php
/**
 * ERGUVAN PSİKOLOJİ - PREMIUM BLOG TASARIMI KURULUMU (V8)
 * Bu dosya canlı sitenizdeki blog yazısı görünümünü profesyonel seviyeye taşır.
 */
...
<article class="blog-post-wrapper">
    <header style="margin-bottom: 3rem; text-align:center;">
        <nav style="margin-bottom: 2rem;"><a href="<?php echo page_url('blog.php'); ?>" style="color:var(--erguvan-pink); text-decoration:none; font-weight:600;">← Blog Listesine Dön</a></nav>
        <span style="background:var(--erguvan-light-pink); color:var(--erguvan-pink); padding: 8px 16px; border-radius:30px; font-weight:700; font-size: 0.9rem; text-transform:uppercase; letter-spacing:1px;"><?php echo htmlspecialchars($post['category']); ?></span>
        <h1 style="font-size: 2.8rem; font-weight: 900; color: #1e293b; margin-top: 1.5rem; line-height: 1.2; font-family: \'serif\';"><?php echo htmlspecialchars($post['title']); ?></h1>
    ...
    <div style="margin-bottom: 4rem;">
        <img src="<?php echo webp_url($post['image']); ?>" style="width:100%; border-radius:30px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.15);" alt="<?php echo htmlspecialchars($post['title']); ?>">
    </div>
...
if (file_put_contents($path, $content)) {
    echo "<h1>BAŞARILI</h1><p>Premium blog tasarımı sisteme başarıyla uygulandı.</p>";
} else {
