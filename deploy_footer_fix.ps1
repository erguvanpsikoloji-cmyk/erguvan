...
Write-Host "DEPLOYING FOOTER FIX..." -ForegroundColor Cyan

# 1. Prepare CSS (Minify and Rename to style2.css locally first to simulate previous steps)
(Get-Content "$sourceRoot\assets\css\style_v38.css") -replace '/\*[\s\S]*?\*/', '' | ForEach-Object { $_.Trim() } | Where-Object { $_ -ne '' } | Set-Content "$sourceRoot\assets\css\style2.css"

# 2. Upload CSS
Publish-File "$sourceRoot\assets\css\style2.css" "assets/css/style2.css"

# 3. Upload Index
Publish-File "$sourceRoot\index_simple_fix.php" "index.php"

Write-Host "FOOTER FIX DEPLOYED!" -ForegroundColor Yellow
